...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 20 February - FSR Experimental Radial Threshold Boundary.");
    Serial.println(" ");
  ...
  xh_yh = sqrt(sq(xh - x_right) + sq(yh - y_up));     // sq() function raises input to power of 2, returning the same data type int->int ...
  xh_yl = sqrt(sq(xh - x_right) + sq(yl - y_down));   // the sqrt() function raises input to power 1/2, returning a float type
  ...
  if (xh_yh > xh_yh_radius || xh_yl > xh_yl_radius || xl_yl > xl_yl_radius || xl_yh > xl_yh_radius) {
    ...
    if (poll_counter >= 3) {
      ...
      if (comm_mode == 0) {
        ...
        if (xh_yh > xh_yh_radius && xl < x_left && yl < y_down && xh > x_right && yh > y_up) {


          if (xh_yh > xh_yh_radius) {
            /*
              if (xh > x_right && yh > y_up){
                //something
              } else if(xh > x_right && yh <= y_up){
                //something
              } else if(xh <= x_right && yh > y_up){
                //something
              }
