...
void Joystick_Initialization(void) {
 ...
 /*
  * 
  * float xh_mod = (float)xh*xh_comp;
  * float xl_mod = (float)xl*xl_comp;
  * float yh_mod = (float)yh*yh_comp;
  * float yl_mod = (float)yl*yl_comp;
  * 
  * float xh_yh_radius = (int)(sqrt(pow((float)xh_mod,2)+pow((float)yh_mod,2)));
  * float xh_yl_radius = (int)(sqrt(pow((float)xh_mod,2)+pow((float)yl_mod,2)));
  * float xl_yl_radius = (int)(sqrt(pow((float)xl_mod,2)+pow((float)yl_mod,2)));
  * float xl_yh_radius = (int)(sqrt(pow((float)xl_mod,2)+pow((float)yh_mod,2)));
  * 
  * THEN in loop() above
  * 
  * float xh_yh = (int)(sqrt(pow((float)xh,2)+pow((float)yh,2)));
  * float xh_yl = (int)(sqrt(pow((float)xh,2)+pow((float)yl,2)));
  * float xl_yl = (int)(sqrt(pow((float)xl,2)+pow((float)yl,2)));
  * float xl_yh = (int)(sqrt(pow((float)xl,2)+pow((float)yh,2)));
  * 
  * if(xh_yh > xh_yh_radius || xh 
  * 
  */
