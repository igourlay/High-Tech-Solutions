...
#define X_DIR_HIGH A1                                   // PIO4 pin - digital output pin A4
#define X_DIR_LOW A2                          // transistor pin - digital output pin A3
#define Y_DIR_HIGH A3                            // joystick x-direction pin - analog input pin A2
#define Y_DIR_LOW A0                            // joystick y-direction pin - analog input pin A1

int xh, yh, xl, yl;                                         // the joystick x and y values
...
_cursor setting1 = {5, -1.0, 10}; // 5,-1.0,10
_cursor setting2 = {5, -1.2, 10}; // 5,-1.2,10
_cursor setting3 = {5, -1.4, 10}; //        10
_cursor setting4 = {5, -1.6, 10}; //        10
_cursor setting5 = {5, -1.8, 10}; //        10
...
void setup() {
  ...
  pinMode(X_DIR_HIGH, INPUT);                    // joystick x pin input
  pinMode(X_DIR_LOW, INPUT);
  pinMode(Y_DIR_HIGH, INPUT);
  pinMode(Y_DIR_LOW, INPUT);                    // joystick y pin input
...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 12 December - Stable version.");
    Serial.println(" --- ");
  ...
  xh = analogRead(X_DIR_HIGH);                    // read x-direction value
  xl = analogRead(X_DIR_LOW);
  yh = analogRead(Y_DIR_HIGH);                    // read y-direction value
...
int y_cursor_high(int j) {
  ...
  int k = (int)(round(1.0 * cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((float)(y_box_up - j)) / y_box_up))))));
...
int y_cursor_low(int j) {
  ...
  int k = (int)(round(-1.0*cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((float)(y_box_down - j)) / y_box_down))))));
...
int x_cursor_high(int j) {
  ...
  int k = (int)(round(-1.0*cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((float)(x_box_right - j)) / x_box_right))))));
...
int x_cursor_low(int j) {
  ...
  int k = (int)(round(cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((float)(x_box_left - j)) / x_box_left))))));
...
void Joystick_Initialization(void) {
  xh = analogRead(X_DIR_HIGH);             // initial resting x position of joystick
  delay(250);
  Serial.println(xh);
  xl = analogRead(X_DIR_LOW);             // initial resting y position of joystick
  delay(250);
  Serial.println(xl);
  yh = analogRead(Y_DIR_HIGH);             // initial resting y position of joystick
  delay(250);
  Serial.println(yh);
  yl = analogRead(Y_DIR_LOW);             // initial resting y position of joystick
  delay(250);
  Serial.println(yl);

  

  box_delta = 15;                                 // ***CAN BE CHANGED TO CREATE MORE SENSITIVE MOVEMENT ACTIVATION *ORIGINALLY 25
                                                  // 10-15 seems to work well for the pre-loaded FSR array

  //^^ COULD BE INCREASED FOR PERSONS WITH SPASTIC OR INADVERTENT MOVEMENTS?

  x_box_right = xh - box_delta;             // create right x threshold
  x_box_left = xl - box_delta;              // create left x threshold

  y_box_up = yh - box_delta;                // create upper y threshold
  y_box_down = yl - box_delta;              // create lower y threshold
}
...
void Pressure_Sensor_Initialization(void) {
  ...
  delay(20);
  sip_threshold = nominal_cursor_value + 1.75;    //create sip pressure threshold value ***Originally: +0.15
  delay(20);
  puff_threshold = nominal_cursor_value - 1.75;   //create puff pressure threshold value ***Originally: -0.15
  delay(20);
}
