...
/*
 * Special revision: modiication to Arduino hardware library to Arduino\hardware\arduino\avr\cores\arduino\HID.cpp
 * Relatively easy fix if given good instructions. Argument for Bluetooth only =/- battery drain?
 */

#include <EEPROM.h>
#include <math.h>  // consider removing this declaration if not math functions are utilized
#include <Mouse.h>
...
//int delta_values[7] = {8, 7, 6, 5, 4, 3, 2};
//int max_speed = 15;
//int min_speed = -15;
...
void setup() {
  ...
  // use area below for serial feedback
...
//***3rd NEW CURSOR MOVEMENT FUNCTIONS***//

int y_cursor_high(int j) {
  int k =  ((int)(((((float)(y_center - j)) / y_center) * 381) / (float)cursor_delta)); //381 = 127*3, ROM is reduced to about 1/3 the normal ROM
  return k;
}
...
int x_cursor_high(int j) {
  int k = (int)(((((float)(j - x_center)) / x_center) * 381) / (float)cursor_delta); //381 = 127*3, ROM is reduced to about 1/3 the normal ROM
  return k;
}
...
int y_cursor_high(int j){
  ...
  //int k = (int)(round((((float)(y_center - j))/y_center) * 2));
  if(k <= min_speed){
    k = min_speed;
  }
  Serial.print("Y:");
...
int x_cursor_high(int j){
  ...
  //int k = (int)(round((((float)(j - x_center))/x_center) * 2));
  if(k >= max_speed){
    k = max_speed;
  }
  Serial.print("X:");
