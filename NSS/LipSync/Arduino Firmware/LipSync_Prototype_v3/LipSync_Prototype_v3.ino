...
#define Y_DIRECTION A1                            // joystick x-direction pin - analog input pin A1
#define X_DIRECTION A2                            // joystick y-direction pin - analog input pin A1
...
_cursor setting1 = {5, -1.0, 7}; // 5,-1.0,10
_cursor setting2 = {5, -1.2, 7}; // 5,-1.2,10
_cursor setting3 = {5, -1.4, 7}; //        10
_cursor setting4 = {5, -1.6, 7}; //        10
_cursor setting5 = {5, -1.8, 7}; //        10
...
void setup() {
  ...
  Serial.begin(4800);                           // setting baud rate for serial coms for diagnostic data return from Bluetooth and microcontroller ***MAY REMOVE LATER***
  Serial1.begin(4800);                          // setting baud rate for Bluetooth module
...
void mouseCommand(int buttons, int x, int y, int scroll) {
  ...
  BTcursor[1] = 0x5;
  BTcursor[2] = 0x2;
  BTcursor[3] = (byte)buttons;
  BTcursor[4] = (char)x;
  BTcursor[5] = (char)y;
  BTcursor[6] = (byte)scroll;
...
void BT_Config_Sequence(void) {                    // change all Serial instructions to Serial1
  ...
  delay(100);
  Serial1.println("SU,48");                   // ***NEW ADDITION - Change baud rate to 57600
  delay(100);
