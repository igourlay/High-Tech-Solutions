...
void setup() {
  ...
  Serial1.begin(115200, SERIAL_8N1);                          // setting baud rate for Bluetooth module
...
void loop() {
    ...
    if (single == 0) {
      ...
      delay(100);
      Serial.println(" --- ");
      Serial.println("This is the 04 November - Experimental Bluetooth version.");
      Serial.println(" --- ");
      single++;
    }
  ...
  if (cursor_click > sip_threshold) {
    ...
    if (comm_mode == 0) {
      ...
      if (sip_count < 150) {
        ...
        cursor_click_status = 0;
        delay(100);
      } else if (sip_count > 150 && sip_count < 300) {
        if (cursor_click_status == 0) {
          cursor_click_status = 2;
...
void mouseCommand(int buttons, int x, int y, int scroll) {
  ...
  delay(10);          // stable at delay(50) but slow on device
}
...
//***BLUETOOTH EXPERIMENTAL HID FUNCTIONS***//
...
void Pressure_Sensor_Initialization(void) {
  ...
  delay(20);
  sip_threshold = nominal_cursor_value + 0.25;    //create sip pressure threshold value ***REVERSED
  delay(20);
  puff_threshold = nominal_cursor_value - 0.25;   //create puff pressure threshold value ***REVERSED
  delay(20);
}
...
void BT_Config_Sequence(void) {                    // change all Serial instructions to Serial1
  ...
  delay(100);
  Serial1.println("SH,0230");                // configure device as HID mouse
  delay(100);
