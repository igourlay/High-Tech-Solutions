...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 19 February - FSR Experimental Radial Threshold Boundary.");
    Serial.println(" ");
  ...
  if (xh_yh > xh_yh_radius || xh_yl > xh_yl_radius || xl_yl > xl_yl_radius || xl_yh > xl_yh_radius) {
    ...
    if (poll_counter >= 3) {
      ...
      if (comm_mode == 0) {
        if ((xh_yh > xh_yh_radius) && ( x_left < xl < x_box_left) && ( y_down < yl < y_box_down)) {
          Mouse.move(x_cursor_high(xh), y_cursor_high(yh), 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if ((xh_yl > xh_yl_radius) && (x_left < xl < x_box_left) && (y_up < yh < y_box_up)) {
          Mouse.move(x_cursor_high(xh), y_cursor_low(yl), 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if ((xl_yl > xl_yl_radius) && (x_right < xh < x_box_right) && yh < y_box_up) {
          Mouse.move(x_cursor_low(xl), y_cursor_low(yl), 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if (xl_yh > xl_yh_radius && xh < x_box_right && yl < y_box_down) {
