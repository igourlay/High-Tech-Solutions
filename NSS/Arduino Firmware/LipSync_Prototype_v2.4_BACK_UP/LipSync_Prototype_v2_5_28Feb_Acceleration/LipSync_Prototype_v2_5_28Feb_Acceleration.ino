...
int xh_a, xl_a, yh_a, yl_a, xh_b, xl_b, yh_b, yl_b;
...
int default_cursor_speed = 10;
int delta_cursor_speed = 2;

unsigned long delta_time_accel = 100;             // time == 10ms -> time^2 == 100ms^2
...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 28 February - FSR Experimental Acceleration Cursor Algorithm");
    Serial.println(" ");
    ...
    */
    ...
    Serial.print("xh_accum: ");
    Serial.print(xh_accum);
    Serial.print(", xl_accum: ");
    Serial.print(xl_accum);
    Serial.print(", yh_accum: ");
    Serial.print(yh_accum);
    Serial.print(", yl_accum: ");
    Serial.println(yl_accum);
  ...
  // experimental acceleration factor calculation

  //READ FIRST SET OF FSR A/D VALUES

  xh_a = analogRead(X_DIR_HIGH);                    // A0 :: NOT CORRECT MAPPINGS
  xl_a = analogRead(X_DIR_LOW);                     // A1
  yh_a = analogRead(Y_DIR_HIGH);                    // A2
  yl_a = analogRead(Y_DIR_LOW);                     // A10
  ...
  delay(50);    //DELIBERATE DELAY SO VALUES ARE NOT COMPUTED EXTREMELY FAST AS WELL AS ALLOWS US TO MODULATE THE ACCELERATION VALUE

  //READ SECOND SET OF FSR A/D VALUES

  xh_b = analogRead(X_DIR_HIGH);                    // A0 :: NOT CORRECT MAPPINGS
  xl_b = analogRead(X_DIR_LOW);                     // A1
  yh_b = analogRead(Y_DIR_HIGH);                    // A2
  yl_b = analogRead(Y_DIR_LOW);                     // A10
  ...
  //COMPUTE DIFFERENCE IN TIME OF SUBSEQUENT A/D VALUE MEASUREMENTS

  //delta_time_accel = (time_stop - time_start) - 45;

  //INCREMENT ACCUMULATOR VARIABLES (ACCUM >= 0 : ALLOW || ACCUM < 0 : RESTRICT)

  xh_accum = xh_accum + (xh_b - xh_a);
  xl_accum = xl_accum + (xl_b - xl_a);
  yh_accum = yh_accum + (yh_b - yh_a);
  yl_accum = yl_accum + (yl_b - yl_a);

  if (xh_accum > 5) {
    xh_accel = (float)(xh_2 - xh_1) / (delta_time_accel);
    Serial.print("xh_accel: ");
    Serial.println(xh_accel);
  } else {
    xh_accel = 0.0;
  }

  if (xl_accum > 5) {
    xl_accel = (float)(xl_2 - xl_1) / (delta_time_accel);
    Serial.print("xl_accel: ");
    Serial.println(xl_accel);
  } else {
    xl_accel = 0.0;
  }

  if (yh_accum > 5) {
    yh_accel = (float)(yh_2 - yh_1) / (delta_time_accel);
    Serial.print("yh_accel: ");
    Serial.println(yh_accel);
  } else {
    yh_accel = 0.0;
  }
  if (yl_accum > 5) {
    yl_accel = (float)(yl_2 - yl_1) / (delta_time_accel);
    Serial.print("yl_accel: ");
    Serial.println(yl_accel);
  } else {
    yl_accel = 0.0;
  }

  //SMALL THRESHOLD TO ALLOW YOU TO DO PLANAR MOVEMENT, IF ACTUALLY NECESSARY
  /*
    if( xh_accel <= 0.0) xh_accel = 0.0;

    if( xl_accel <= 0.0) xl_accel = 0.0;

    if( yh_accel <= 0.0) yh_accel = 0.0;

    if( yl_accel <= 0.0) yl_accel = 0.0;
  */

  if (xh_accel > xl_accel) {
    if (yh_accel > yl_accel) {
      Mouse.move(x_cursor_high(xh_accel), y_cursor_high(yh_accel), 0);
    } else if (yh_accel < yl_accel) {
      Mouse.move(x_cursor_high(xh_accel), y_cursor_low(yl_accel), 0);
    } else if (yh_accel == yl_accel) {
      Mouse.move(x_cursor_high(xh_accel), 0, 0);
    }
  } else if (xh_accel < xl_accel) {
    if (yh_accel > yl_accel) {
      Mouse.move(x_cursor_low(xl_accel), y_cursor_high(yh_accel), 0);
    } else if (yh_accel < yl_accel) {
      Mouse.move(x_cursor_low(xl_accel), y_cursor_low(yl_accel), 0);
    } else if (yh_accel == yl_accel) {
      Mouse.move(x_cursor_low(xl_accel), 0, 0);
    }
  } else if ((xh_accel == xl_accel) && ((yh_accel > yl_accel) || (yh_accel < yl_accel))) {
    if (yh_accel > yl_accel) {
      Mouse.move(0, y_cursor_high(yh_accel), 0);
    } else if (yh_accel < yl_accel) {
      Mouse.move(0, y_cursor_low(yl_accel), 0);
    } else if (yh_accel == yl_accel) {
      //do nothing!
    }
  }
...
int y_cursor_high(float j) {

  int cursor_speed = (int)(round(-1.0 * (float)cursor_max_speed * j));
...
/*
  ...
  if (j > y_up) {
    ...
    float y_up_factor = 3.0 * round((yh_comp*((float)(j-y_up))/y_up));
...
/*
  ...
  if (j > y_down) {
    ...
    float y_down_factor = 1.75 * ((yl_comp*(float)(j-y_down))/y_down);
...
/*
  ...
  if (j > x_right) {
    ...
    float x_right_factor = 1.75 * ((xh_comp*(float)(j-x_right))/x_right);
...
/*
  ...
  if (j > x_left) {
    ...
    float x_left_factor = 1.75 * ((xl_comp*(float)(j-x_left))/x_left);
...
void Joystick_Initialization(void) {
  ...
  constant_radius = 20.0;  //original value was 30.0 as of 28 Feb
