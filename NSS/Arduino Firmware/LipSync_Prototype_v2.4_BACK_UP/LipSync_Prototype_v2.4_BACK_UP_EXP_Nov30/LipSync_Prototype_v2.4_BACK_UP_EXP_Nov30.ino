...
void BT_Command_Mode(void) {                 //***CHANGE THE TRANSISTOR CONTROLS ONCE THE PNP IS SWITCHED IN FOR THE NPN***
  ...
  delay(100);                               //***Need about 100ms to allow BT to actually power up

  for (int i = 0; i < 3; i++) {             // cycle PIO4 pin high-low 3 times with 1 sec delay between each level transition
    digitalWrite(PIO4, HIGH);
    delay(100);                             // usually 1 secoond intervals but it works with 100ms
...
void change_Baud(void){
  BT_Command_Mode();
  Serial1.println("SU,96");                  // ***NEW*** set baud rate to 9600 - see if this changes the stutter problem
  delay(100);
  Serial1.println("R,1");
  delay(100);
  Serial1.end();
