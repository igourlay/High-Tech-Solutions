...
int poll_counter = 0;                             // mouse data polling counter
...
void setup() {
  ...
  delay(2000);   //***Needed to establish serial comms - in final build any serial feedback may not be necessary???
...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 24 December - Fast Start-Up Sequence version.");
    Serial.println(" --- ");
    ...
    Serial.println(analogRead(X_DIRECTION));
    delay(10);
    Serial.println(analogRead(Y_DIRECTION));
    delay(10);
...
void mouseCommand(int buttons, int x, int y, int scroll) {
  ...
  BTcursor[1] = 0x5;
  BTcursor[2] = 0x2;
  BTcursor[3] = /*0b00000000; //*/lowByte(buttons);
  BTcursor[4] = /*0b00000000; //*/lowByte(x);
  BTcursor[5] = /*0b00000010; //*/lowByte(y);
  BTcursor[6] = /*0b00000000; //*/lowByte(scroll);
...
void BT_Command_Mode(void) {                 //***CHANGE THE TRANSISTOR CONTROLS ONCE THE PNP IS SWITCHED IN FOR THE NPN***
  ...
  delay(10);                               //***Need about 100ms to allow BT to actually power up

  for (int i = 0; i < 3; i++) {             // cycle PIO4 pin high-low 3 times with 1 sec delay between each level transition
    digitalWrite(PIO4, HIGH);
    delay(75);                             // usually 1 secoond intervals but it works with 100ms
...
void BT_Config_Sequence(void) {                    // change all Serial instructions to Serial1
  ...
  //Serial1.println("SU,11");                // BE CAREFUL CHANGING BAUD RATE!!!
  //delay(100);
...
void Set_Default(void) {      // This function should only occur ONCE when this code is first uploaded to the microcontroller
  int ral0;
  int ral1;
  int ral2;
...
void fast_data_mode(void) {
  BT_Command_Mode();
  delay(100);
  Serial1.println("CFI");                  // ***NEW*** set baud rate to 9600 - see if this changes the stutter problem
  delay(100);
  Serial1.println("---");
  delay(100);
  BT_configAOK();
