...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 18 February - FSR Experimental Radial Threshold Boundary.");
    Serial.println(" ");
  ...
  float xh_yh = (int)(sqrt(pow((float)(xh-x_right),2)+pow((float)(yh-y_up),2)));
  float xh_yl = (int)(sqrt(pow((float)(xh-x_right),2)+pow((float)(yl-y_down),2)));
  float xl_yl = (int)(sqrt(pow((float)(xl-x_left),2)+pow((float)(yl-y_down),2)));
  float xl_yh = (int)(sqrt(pow((float)(xl-x_left),2)+pow((float)(yh-y_down),2)));
    ...
    if (poll_counter >= 3) {
      ...
      if (comm_mode == 0) {
        if (xh_yh > xh_yh_radius ){//&& (xh_yl < xh_yl_radius && xl_yl < xl_yl_radius && xl_yh < xl_yh_radius)) {
          ...
          Mouse.move(x_cursor_low(xl), y_cursor_high(yl), 0);
          delay(cursor_delay);
          poll_counter = 0;
...
void Joystick_Initialization(void) {
  ...
  float xh_mod = (float)box_delta*xh_comp;
  float xl_mod = (float)box_delta*xl_comp;
  float yh_mod = (float)box_delta*yh_comp;
  float yl_mod = (float)box_delta*yl_comp;
  
  xh_yh_radius = 25;//(int)((xh_mod+yh_mod)/2.0);
  xh_yl_radius = 25;//(int)((xh_mod+yl_mod)/2.0);
  xl_yl_radius = 25;//(int)((xl_mod+yl_mod)/2.0);
  xl_yh_radius = 25;//(int)((xl_mod+yh_mod)/2.0);
...
/*
  ...
  if (yh_max >= yl_max ? max1 = yh_max : max1 = yl_max);

  if (xh_max >= xl_max ? max2 = xh_max : max2 = xl_max);

  if (max1 >= max2 ? abs_max = max1 : abs_max = max2);
*/
  ...
  /*
    yh_comp = (abs_max - yh) / (yh_max - yh);
    yl_comp = (abs_max - yl) / (yl_max - yl);
    xh_comp = (abs_max - xh) / (xh_max - xh);
    xl_comp = (abs_max - xl) / (xl_max - xl);
  */
...
void Auto_Home_Calibration(void) {
  ...
  x_right = xh;             // Create horizontal right x-direction threshold
  x_left = xl;              // Create horizontal left x-direction threshold

  y_up = yh;                // Create vertical up y-direction threshold
  y_down = yl;              // Create vertical down y-direction threshold
