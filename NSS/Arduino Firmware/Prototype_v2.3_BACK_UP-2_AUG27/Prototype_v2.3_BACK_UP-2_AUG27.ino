...
int delta_values[8] = {5, 5, 10, 15, 20, 25, 30, 35};
...
void setup() {
  ...
  blink(3, 1000, 1);                                 // some visual feedback
  delay(250);
  Serial.print("config_done == ");
  Serial.println(EEPROM.get(2, puff1));
  delay(250);
  Serial.print("speed_counter: ");
  Serial.println(EEPROM.get(4, puff2));
  delay(250);
  delay(250);
  Serial.print("cursor_delta: ");
  Serial.println(delta_values[EEPROM.get(4, puff2)]);
  delay(250);
}
...
void BT_Config_Sequence(void) {                    // change all Serial instructions to Serial1
  ...
  int val1 = 4;
  delay(250);
  EEPROM.put(2, val0);                        // EEPROM address 0 gets configuration completed value (== 1)
  delay(500);
  delay(100);
  EEPROM.put(4, val1);                        // EEPROM address 1 gets default cursor speed counter value (== 20) ***SHOULD ONLY OCCUR ONCE UNLESS THE LIPSYNC IS FACTORY RESET??
  delay(500);
  delay(100);
}
...
void set_default_values(void) {                   // SHOULD ONLY OCCUR ONCE IN THE WHOLE LIFETIME OF LIPSYNC UNLESS factory_reset() FUNCTION IS CALLED
  ...
  Serial.println(var0);
  EEPROM.get(0, var0);
  Serial.println(EEPROM.get(0, var0));
  Serial.println(var0);
  if (var0 != 0 && var0 != 1) {
    var0 = 0;
    EEPROM.put(0, var0);
...
void joystick_initialization(void) {
  ...
  box_delta = 25;                                 // ***CAN BE CHANGED TO CREATE MORE SENSITIVE MOVEMENT ACTIVATION
